...
if(process.platform == 'win32'){
    ...
    // Edge
    var winver = os.release();
    if(winver.split('.')[0] == '10'){
        if(parseInt(winver.split('.')[2]) < 14393){ //msi
            ...
            var edgedriverpathsuffix = 'edgedriver';
            var edgedriverfolder = path.join(seleniumdriverpath,edgedriverpathsuffix);
            var edgedriverbinary = getFirstNonZip(edgedriverfolder);
            edgedriverconfigvalue = (edgedriverbinary ? path.join(seleniumdriverpath,edgedriverpathsuffix,edgedriverbinary) : '');
        }
